...
import { columnDef } from "../Columns/StudentsGuidedColumn";
import "../../table.css";
...
export default function AdminStudentsGuidedTable() {
  const { id } = useParams();
  // console.log(id);
  ...
  useEffect(() => {
    const fetchTeacherInfo = async () => {
      try {
        ...
        const response = await axios.get(
          `https.facultyappraisal.software/api/v1/admins/teachers/${id}/students-guided/mtech`,
          {
            headers: {
  ...
  //         ),
  //         axios.get(
  //           `https.facultyappraisal.software/api/v1/admins/teachers/${id}/students-guided/phd`,
  //           {
  //             headers: {
  //               Authorization: `Bearer ${token}`,
  //             },
