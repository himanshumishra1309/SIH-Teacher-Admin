  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        feedbackForms,
        "Feedback forms released for all subjects successfully"
      )
    );
});
...
const getReleasedFeedbackForms = asyncHandler(async (req, res) => {
  ...
  const enrolledSubjects = await StudySubject.find({ owner: studentId }).select(
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, feedbackForms, "Feedback forms fetched successfully")
    );
});
...
const fillFeedbackForm = asyncHandler(async (req, res) => {
  ...
  const feedbackForm = await FeedbackForm.findById(feedbackFormId).populate({
    path: "subject",
  ...
  if (
    student.branch !== feedbackForm.subject.branch ||
    student.year !== feedbackForm.subject.year
  ) {
    throw new ApiError(
      403,
      "Your branch or year does not match the subject's requirements"
    );
  }
...
const getFeedbackForSubject = asyncHandler(async (req, res) => {
  const { subject_name, subject_code, subject_credit, branch, year } =
    req.params; // Extract parameters from request
  ...
  if (!subject) {
    throw new ApiError(
      404,
      "No subject found for this teacher with the provided details."
    );
  }
...
const getStudentDetails = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, studentList, "Student details fetched successfully.")
    );
});
