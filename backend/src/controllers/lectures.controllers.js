...
const addNewLecture = asyncHandler(async (req, res)=>{
    ...
    return res.status(200).json(new ApiResponse(200, lecture, "New Lecture Created Successfully"))
});
...
const editLecture = asyncHandler(async (req,res)=>{
    ...
    return res.status(200).json(new ApiResponse(200, editedLecture, "Lecture Edited Successfully"))
});
...
const deleteLecture = asyncHandler(async (req, res)=>{
    ...
    return res.status(200).json(new ApiResponse(200, deletedLecture, "Lecture Deleted Successfully"))
});
...
const getLectureById = asyncHandler(async (req, res)=>{
    ...
    const lecture = await Lecture.find({subject: subjectId, teacher: teacherId});

    if(!lecture){
        throw new ApiError(404, "Lecture Not Found");
    }

    return res.status(200).json(new ApiResponse(200, lecture, "Lecture Found Successfully"))
});
...
const markLectureAttendance = asyncHandler(async (req, res) => {
    const { lectureId } = req.params;
    const { studentIds, subject_name, subject_code, subject_credit, branch, year, date } = req.body;
    ...
    if (existingAttendance) {
        throw new ApiError(400, "Attendance already marked for this lecture on the specified date");
    }
    ...
    return res.status(200).json(
        new ApiResponse(200, { attendance, markedStudents: validStudentIds.length }, "Attendance Marked Successfully")
    );
});
