...
import { SeminarRSVP } from "../models/seminar-rsvp.model.js";
...
const getAllConductedSeminars = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, conductedSeminars, "Conducted seminars fetched successfully"));
});
...
const postUpcomingSeminar = asyncHandler(async (req, res) => {
  ...
  return res.status(201).json(new ApiResponse(201, newSeminar, "Upcoming seminar posted successfully"));
});
...
const seeFeedbacks = asyncHandler(async (req, res) => {
  ...
  const seminar = await Seminar.findOne({ _id: seminarId, owner: req.user._id, status: 'conducted' });
  if (!seminar) {
...
const editConductedSeminar = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, seminar, "Seminar updated successfully"));
});
...
const deleteConductedSeminar = asyncHandler(async (req, res) => {
  ...
  const seminar = await Seminar.findOneAndDelete({ _id: seminarId, owner: req.user._id, status: 'conducted' });
  if (!seminar) {
    throw new ApiError(404, "Conducted seminar not found or not owned by you");
  }
  ...
  return res.status(200).json(new ApiResponse(200, null, "Conducted seminar deleted successfully"));
});
...
const seeRSVPedStudents = asyncHandler(async (req, res) => {
  ...
  const seminar = await Seminar.findOne({ _id: seminarId, owner: req.user._id, status: 'upcoming' });
  if (!seminar) {
  ...
  return res.status(200).json(new ApiResponse(200, rsvps, "RSVPed students fetched successfully"));
});
...
const seeFeedbackSubmitters = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, feedbacks, "Feedback submitters fetched successfully"));
});
...
const getAllUpcomingSeminarsForStudents = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, seminars, "Upcoming seminars fetched successfully"));
});
...
const getAllUpcomingSeminarsByTeacher = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, seminars, "Your upcoming seminars fetched successfully"));
});
...
const conductSeminar = asyncHandler(async (req, res) => {
  ...
  // Implement logic to notify RSVPed students to submit feedback if needed

  return res.status(200).json(new ApiResponse(200, seminar, "Seminar marked as conducted and feedback forms released"));
});
...
const studentRSVP = asyncHandler(async (req, res) => {
  ...
  const existingRSVP = await SeminarRSVP.findOne({ seminar: seminarId, student: req.user._id });
  if (existingRSVP) {
...
const studentSubmitFeedback = asyncHandler(async (req, res) => {
  ...
  const seminar = await Seminar.findOne({ _id: seminarId, status: 'conducted' });
  if (!seminar) {
    throw new ApiError(404, "Conducted seminar not found");
  }

  const rsvp = await SeminarRSVP.findOne({ seminar: seminarId, student: req.user._id });
  if (!rsvp) {
  ...
  return res.status(201).json(new ApiResponse(201, null, "Feedback submitted successfully"));
});
